...
''')
    ...
    def generate_target(self, target):
        ...
        # Removes from all_source_files files that gets transpiled (e.g. *.vala, *.pyx)
        # and add generated C files.
        self.generate_target_transpiles(target, all_source_files, all_other_files, header_deps)
    ...
    def generate_target_transpiles(self, target: 'build.BuildTarget',
                                   target_sources: T.List[T.Tuple[File, T.Union[bool, str]]],
                                   target_others: T.List[File],
                                   order_deps: T.List[File]) -> None:
        for lang, comp in target.compilers.items():
            suffixes = comp.get_transpile_suffixes()
            if not suffixes:
                continue

            # Remove source file transpiled by this compiler out of
            # target_sources and put them in transpile_sources.
            transpile_sources: T.List[File] = []
            ...
            for f, is_generated in target_sources:
                if any(f.endswith(f'.{s}') for s in suffixes):
                    transpile_sources.append(f)
                else:
                    other_sources.append((f, is_generated))
            target_sources[:] = other_sources

            # Transpile sources and add the output back into target_sources
            if lang == 'vala':
                ...
                vapis = [f for f in target_others if f.endswith('.vapi')]
    ...
    def generate_vala_compile(self, target: build.BuildTarget,
                              ...
                              target_vapis: T.List[File]) -> T.List[str]:
        ...
        all_files = [s.rel_to_builddir(self.build_to_src) for s in target_vapis]
        # Passed as --basedir
