...
@T.overload
def _fixup_raw_mappings(d: manifest.Dependency) -> manifest.FixedDependency: ...  # type: ignore

@T.overload
def _fixup_raw_mappings(d: manifest.Package) -> manifest.FixedPackage: ...

def _fixup_raw_mappings(d: T.Union[manifest.BuildTarget, manifest.Dependency, manifest.Package]
                        ) -> T.Union[manifest.FixedBuildTarget, manifest.FixedDependency,
                                     manifest.FixedPackage]:
    """Fixup raw cargo mappings to ones more suitable for pythong to consume.
    ...
    * Convert Dependndency versions from the cargo format to something meson
    ...
    return T.cast('T.Union[manifest.FixedBuildTarget, manifest.FixedDependency, manifest.FixedPackage]', raw)
...
def _convert_manifest(raw_manifest: manifest.Manifest, subdir: str, path: str = '') -> Manifest:
    ...
    lib = T.cast('manifest.FixedLibTarget', _fixup_raw_mappings(raw_manifest.get('lib', {})))
